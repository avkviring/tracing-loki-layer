...
impl LokiLayer {
    pub fn new<S: AsRef<str>>(url: S, default_values: HashMap<String, String>) -> Self {
    ...
    fn on_event(&self, event: &Event<'_>, _ctx: Context<'_, S>) {
        ...
        match self.sender.lock() {
            Ok(sender) => match sender.send(request) {
                Ok(_) => {}
                Err(e) => {
                    eprintln!("Loki sender thread is closed {:?}", e);
                }
            },
            Err(e) => {
                eprintln!("Cannot lock mutex for send to loki {:?}", e);
            }
        }
    }
}
